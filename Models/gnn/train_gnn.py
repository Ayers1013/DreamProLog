...
def eval_epoch(network, data_pos, data_neg, balance, batch_size, train=False):
    ...
    total_len, loss_pol_total, loss_val_total = 0, 0, 0
    if train:
        phase = "Training"
    else:
        phase = "Evaluation"
    with StopWatch(phase):
        while True:
            ...
            if train:
                loss_pol, loss_val = network.train_supervised(states)
            else:
                loss_pol, loss_val = network.get_loss_supervised(states)
    ...
    return train_loss_pol, train_loss_val


def train(train_dirs, model_file, epochs, batch_size, remove_duplicates):
    ...
    loss_pol_train, loss_val_train = eval_epoch(network, data_pos_train, data_neg_train, balance=False, batch_size=batch_size, train=False)
    loss_pol_test, loss_val_test = eval_epoch(network, data_pos_test, data_neg_test, balance=False, batch_size=batch_size, train=False)
    print("policy {} / {}  -  value {} / {}\n".format(loss_pol_train, loss_pol_test, loss_val_train, loss_val_test))
    ...
    for epoch in range(epochs):
        ...
        loss_pol_train, loss_val_train = eval_epoch(network, data_pos_train, data_neg_train, balance=True, batch_size=batch_size, train=True)
        loss_pol_test, loss_val_test = eval_epoch(network, data_pos_test, data_neg_test, balance=False, batch_size=batch_size, train=False)
        print("Epoch {}: policy {} / {}  -  value {} / {}".format(epoch+1, loss_pol_train, loss_pol_test, loss_val_train, loss_val_test))
        sys.stdout.flush()

        current_val_loss = loss_pol_test + loss_val_test
