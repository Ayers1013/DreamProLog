...
class Net(tf.keras.Model):
    def __init__(self, N=4, querry=8, output_length=128, d_model=128, num_heads=4, dff=256, rate=0.04):
        ...
        self.mha_layers = [MultiHeadAttention(d_model, num_heads) for i in range(N)]
        self.sl_layers = [SimpleLayer(d_model, dff, rate) for i in range(N)]
    ...
    def call(self, x, training):
        ...
        for mha, sl in zip(self.mha_layers, self.sl_layers):
            _x, _ = mha(x, x, x, mask)
            x = x+_x
            x = sl(x)
