...
class Controller(ConfiguredModule):
  def __init__(self, **kwargs):
    ...
    self.datasetManager = DatasetManager(self.logger, self.get_signature, self.traindir, self.evaldir)
    
    make = lambda mode: make_env(self, self.datasetManager.get_callbacks(mode, self))
  ...
  def preinit(self):
    ...
    self.traindir = self.traindir or self.logdir / 'train_eps'
    self.evaldir = self.evaldir or self.logdir / 'eval_eps'
    self.act = getattr(tf.nn, self.act)
    ...
    elif self.logger == 'empty':
      logger = tools.LoggerEmpty(logdir, step, self) 

    return logger

  def prefill_dataset(self):
    prefill = max(0, self.prefill - count_steps(self.traindir))
    ...
    self.logger.step = count_steps(self.traindir)
