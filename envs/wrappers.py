...
class CollectDataset:
  ...
  def step(self, action):
    ...
    _obs = {k: self._convert(v) for k, v in obs.items() if k!='gnn'}
    if 'gnn' in obs.keys(): _obs['gnn']=obs['gnn']
    obs=_obs
    transition = obs.copy()
  ...
  def reset(self):
    ...
    transition = obs.copy()
    #NOTE I am not sure 
    transition['action'] = 0 #np.zeros(self._env.action_space_size)
