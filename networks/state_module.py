...
from transformer import RegressiveModel, Decoder, Encoder, RegressiveCategoricalLoss, CustomSchedule
...
class StateNet(tf.Module):
    ...
    num_heads = 8, dropout_rate = 0.1, d_scale = 4):
        ...
        self.goal_autoencoder = Model('regressive',
            N = goal_N, embed_tokens = embed_tokens, querry = goal_querry, output_length = length, 
            d_model = d_model, num_heads = num_heads, dff = dff, rate = dropout_rate
        )
        self.state_autoencdoer = RegressiveAutoencoder(
    ...
    def encode(self, inp, training):
        ...
        goal_output, goal_masks = self.goal_autoencoder.encode(x, training)

        goal_latent = goal_output[0]
        x = goal_latent.extract(training)
        x = tf.reshape(x, (batch_size, 128, self.goal_querry * 128))
        x = self.latent_dense(x)
        
        state_latent = self.state_autoencoder(x, state_masks[0], training)
    ...
    def calc_loss(self, inp, target, training):
        ...
        goal_latent_reconst_sample = goal_latent.reconst.extract(training)
        goal_latent_reconst_sample = self.out_dense(goal_latent_reconst_sample)
        goal_latent_reconst_sample = tf.reshape(goal_latent_reconst_sample, (batch_size*128,) + goal_latent_reconst.shape[2:])
        ...
        losses['state_to_state'] = state_outputs[0].comparison_loss(goal_latent_reconst)
