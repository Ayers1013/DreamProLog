...
import numpy as np
from gnn_keras.debug_node import tf_debug
from gnn_keras.tf_helpers import *
...
class SegmentsPH(Segments):
    def __init__(self, data_shape = (), dtype = tf.int32, nonzero = False):
        ...
        Segments.__init__(self, tf.keras.Input(shape=(), name="segment_lens", dtype=tf.int32), nonzero = nonzero)

        if data_shape is None:
            self.data = None
        else:
            self.data_shape = tuple(data_shape)
            self.data = tf.keras.Input(shape=(None,)+self.data_shape, name="segment_data", dtype=dtype)
